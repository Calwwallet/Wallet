...
export async function getPrice(name, durationYears = 1, chain = 'ethereum') {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.warn(`⚠️ Coinbase API returned ${res.status}. Using fallback ETH price: $${ethPrice}`);
    }
  } catch (error) {
    console.warn('⚠️ Failed to fetch ETH price from Coinbase:', error.message);
  }
...
export async function prepareRegistration({ name, ownerAddress, durationYears = 1, chain = 'ethereum' }) {
  ...
  const registration = {
    ...
    durationYears,
    secret,
    estimatedPrice: JSON.stringify(price),
    chain,
    createdAt: new Date().toISOString(),
    status: 'ready'
  };

  db.prepare('INSERT INTO ens_registrations (name, label, owner, chain, secret, data, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(
    registration.name, registration.label, registration.owner, registration.chain, registration.secret, registration.estimatedPrice, registration.status, registration.createdAt
  );
...
export function listRegistrations() {
  const rows = db.prepare('SELECT * FROM ens_registrations').all();
  return rows.map(r => ({
    ...JSON.parse(r.data || '{}'),
    name: r.name,
    label: r.label,
...
export function getRegistration(name) {
  const label = name.toLowerCase().replace('.eth', '');
  const row = db.prepare('SELECT * FROM ens_registrations WHERE label = ? OR name = ?').get(label, `${label}.eth`);
  if (!row) return null;
  return {
    ...JSON.parse(row.data || '{}'),
    name: row.name,
